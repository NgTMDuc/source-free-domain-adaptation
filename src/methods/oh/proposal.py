...
def train_target(cfg):
    ...
    while iter_num < max_iter:
        ...
        # softmax of source time t model
...
class LabelProgationCluster(nn.Module):
    def __init__(self, classification_weight, dataset_size, k = 10, alpha = 0.99, cut_dim = 768):
        ...
        # containter for pseudo labels, fetures, etc
        ...
        self.update_centriods(classification_weight.t())
    
    def forward(self, x, idx, label):
        # update feature into memory
    ...
    def perform_label_propagation(self, clear_cache = True, cluster_centriod = False):
        self.all_feat_stack = torch.cat(self.all_feat, dim = 0)
        ...
        # Using scipy to solve linear system pW = Y
        nW = nW.numpy()
        ...
        # perform cfg optimization
        _, out_sims = self.cg_diffusion(Y, nW, self.alpha)
        
        # prediction 
        prediction = np.argmax(out_sims, axis = 1)
        
        # entropy
        out_sims_normalized = (out_sims.T / out_sims.sum(axis=1)).T
